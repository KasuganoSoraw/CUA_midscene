...
`showui-aloha` 当前只作为教学录制处理器：读取录制资源，抽取截图和交互日志，并生成语义 trace。`CUA_midscene` 是执行侧：初始化 Midscene computer use，并包含当前的手写实验脚本，例如 `aloha-air-tickets.ts`。
...
回滚方式直接：删除新增项目资产和 converter/runner 代码。现有手写 Midscene 脚本可以继续独立运行。
