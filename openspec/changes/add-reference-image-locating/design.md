...
### 2. trace 对无文字小图标使用直接规则

click/doubleClick operation 可选输出 `useReferenceImage: true`。字段缺失或 false 表示不使用。

实际样本表明，“并且纯文字 prompt 仍易混淆”要求弱模型做了额外的主观判断，导致无文字图标也普遍不启用参考图。规则调整为：红叉下方目标本体是紧凑的纯图标或符号控件，且目标本身没有可见文字标签时，模型必须输出 `useReferenceImage: true`。典型目标包括无文字的关闭、设置、更多、告警、查看、展开/收起等图标按钮。

带稳定文字的按钮、链接、菜单项、页签、列表项，以及带相邻可见标签的单选框或复选框，不因内部含有小图形而启用参考图。这里仍不增加 `isSmallIcon` 字段；`useReferenceImage` 直接表达执行时是否附加图片。模型也不输出路径、图片名、Base64、置信度或兜底动作。
...
- [弱模型可能把带文字的紧凑控件误判为纯图标] → prompt 明确要求判断红叉下方目标本体，并列出文字按钮、带标签选择控件等反例；canonical YAML 仍可由人或 Agent 按既有确认协议校准。
