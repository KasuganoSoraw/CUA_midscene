...
Midscene 1.10.0 的报告根目录只公开为 `MIDSCENE_RUN_DIR` 环境配置，没有发现实例级目录参数。因此第一版同进程执行使用进程级互斥锁：进入执行临界区后设置本次绝对目录，在 `finally` 中销毁 Agent 并恢复原环境值。并发调用必须排队，不能共享或覆盖报告目录；如果后续 Midscene 提供实例级参数，再移除该临界区。
...
- [Midscene 报告目录依赖进程环境，限制同进程并发] → 使用显式互斥锁和 `finally` 恢复环境；在 API 文档中声明电脑操作当前串行执行。
