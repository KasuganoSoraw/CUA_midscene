...
### Requirement: 真实电脑操作独占执行
系统 SHALL 将本地电脑视为单一独占执行资源，所有创建 ComputerAgent 的逐 task 与整体 aiAct 调用在同一时刻最多只能运行一个。

#### Scenario: 多个调用方同时请求操作电脑
- **WHEN** 一个 computer use 调用尚未结束且另一个实际执行请求到达
- **THEN** 后续请求 SHALL 等待独占执行槽
- **AND** 两个 ComputerAgent SHALL NOT 同时操作键盘、鼠标或屏幕

#### Scenario: 执行失败或异常结束
- **WHEN** 持有执行槽的调用成功、失败或抛出异常
- **THEN** 系统 SHALL 在 `finally` 中销毁 Agent、恢复本次修改的环境并释放执行槽

#### Scenario: 静态任务操作
- **WHEN** 调用方执行发现、转换、validate、inspect 或 dry-run
- **THEN** 系统 SHALL NOT 获取电脑执行槽
- **AND** 这些操作 SHALL NOT 因电脑执行槽被占用而阻塞
