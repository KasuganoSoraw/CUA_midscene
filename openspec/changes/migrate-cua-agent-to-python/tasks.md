...
- [x] 5.1 让 Review Server 仅在 `--dev` 下启动/调用 Python Agent，并移除外部 Agent Host 可用性依赖
- [x] 5.2 调整 Agent 调试页展示自然语言任务、结构化状态、最终回复和允许公开的事件
- [x] 5.3 验证普通模式隐藏、开发模式调用、失败/取消显示及服务复用
