...
未配置 `CUA_RECORDINGS_ROOT` 时，录制页只显示变量名、`execution/.env.local` 配置位置、绝对路径示例和重启提示。review 不提供写环境文件的 API，也不从后台服务进程唤起桌面模态框。

标准浏览器目录选择 API 不暴露可供 Python 录制器和后端复用的绝对路径，而后台进程唤起的 WinForms/AppleScript/Zenity 对话框存在焦点、可见性和请求长期挂起问题。环境配置继续由 Agent、启动进程或用户显式维护，review 页面只消费配置结果。

### 7. 下拉控件共享同一视觉语言

纯选择字段使用可复用的本地 `ReviewSelect`，需要“选择已有值或输入新值”的场景字段使用本地可编辑 Combobox。当前页面只需要少量下拉字段，不引入完整通用组件库。二者共享字段高度、圆角、边框、背景、右侧内边距、焦点环、下拉箭头、弹层阴影与选项状态。箭头是轻量的状态图标，不单独铺设大面积 hover 背景；hover 和 focus 状态作用于完整字段边界，保证右下角圆角和衬线始终可见。`ReviewSelect` 保持 select-only combobox 的单选语义，并支持方向键、Enter/Space 和 Escape。
