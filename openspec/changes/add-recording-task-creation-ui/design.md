...
纯选择字段使用 Element Plus Select，需要“选择已有值或输入新值”的场景字段使用其 filterable/allow-create 模式，避免继续维护自制 Combobox。二者共享字段高度、圆角、边框、背景、右侧内边距、焦点环和下拉箭头尺寸。箭头是轻量的状态图标，不单独铺设大面积 hover 背景；hover 和 focus 状态作用于完整字段边界，保证右下角圆角和衬线始终可见。
