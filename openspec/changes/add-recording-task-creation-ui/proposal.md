...
- 未配置录制根时允许用户在页面内输入本地录制集合的绝对路径，由服务端校验后安全更新 `execution/.env.local`，供当前与后续 review 服务使用。
...
- 统一任务复核页与录制创建页中原生 Select 和可编辑 Combobox 的字段高度、边框、尾部箭头、悬浮态与焦点态，箭头区域不得用大面积遮罩破坏输入框圆角。
