...
- [x] 11.1 以首张桌面帧的单调时钟作为共享原点，按真实经过时间映射 CFR 帧槽并复制上一帧补齐采集缺口
- [x] 11.2 录制停止时补齐视频到单调停止时刻，并为低于目标采集帧率、向上帧槽映射和会话起点增加回归测试
- [x] 11.3 点击保留 release 动作时间并新增第一次 mouse-down 证据时间，截图严格取证据时间前 0.1 秒且在零处截断
- [x] 11.4 双击清理时继承第一次按下证据时间，并通过录制器与 Aloha_Learn 处理器测试
