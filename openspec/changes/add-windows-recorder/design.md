...
Worker 使用 `time.perf_counter_ns()` 作为视频/输入共享的单调基准，并以第一张桌面帧到达时刻作为共同原点。视频保持标准 CFR，但 PTS 不再直接使用采集帧序号：每张真实帧按单调经过时间向上映射到 30fps 帧槽，缺失槽复制上一张已知帧，停止时补齐到单调停止时刻。这样采集不足 30fps 时表现为短暂静止，而不会压缩整段视频并与输入日志持续漂移；向上取整也避免把未来画面提前放入过去的帧槽。

只有 PyAV 捕获首帧并成功提交编码后才安装输入 Hook 并输出 `recording` 状态。点击动作继续以 release 时间表示动作完成顺序，但额外保存以第一次 mouse-down 为基准的证据时间；截图使用 `max(0, evidence_timestamp - 0.1s)`。双击清理单击动作时必须继承第一次 mouse-down，避免截到第一次点击后的状态。停止时先冻结输入、排空队列，再结束捕获循环、补帧、flush 编码器并关闭 MP4 container；关闭失败时以失败退出。
