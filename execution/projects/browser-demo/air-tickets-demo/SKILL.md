...
本任务的长期执行流程是同目录下的 `task.yaml`，输入 ID 和录制默认值以 `task.json` 为准。
...
用户指出某个动作以后都应修改时，先展示 `task.yaml` 原值、新值和中文原因，等待明确确认。确认后编辑 `task.yaml`，再运行 `uv run cua task validate --scene browser-demo --task air-tickets-demo`。
