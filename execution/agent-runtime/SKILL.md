...
- 纯 ASCII 文本由 `KeyboardTypeText` 通过键盘事件输入，不使用剪贴板；只有待输入文本包含中文等键盘动作不支持的字符时，aiAct 才允许使用 Midscene 默认 `Input`。默认 `Input` 依赖剪贴板，在堡垒机等剪贴板隔离环境中可能失败。
- 不得因为定位失败、输入失败或一般执行失败而将默认 `Input` 作为 `KeyboardTypeText` 的兜底。
