...
Python CLI 生成 `resolved-flow.json`；TypeScript executor 只读取该文件并写入 `execution-result.json`。executor 不读取 `task.json`、canonical flow、source 或单次参数，也不实现 resolver 兜底。
