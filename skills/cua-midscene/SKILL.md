...
定位同时包含 `execution/pyproject.toml` 和 `execution/package.json` 的 CUA 仓库，并从 `execution` 目录运行 Python CLI。Python 负责业务契约；TypeScript 只消费 prompt 或 resolved flow 并调用 Midscene。
