...
输入日志使用 Aloha 的物理按键 press/release、鼠标按下/释放、双击、滚轮与拖拽消息格式。英文字母会在录制时结合 Shift 与 CapsLock 状态写入真实大小写；数字行和 OEM 标点键按照标准美式键盘布局结合 Shift 写入对应字符。该静态映射不还原其他键盘布局或中文输入法最终提交文本。
